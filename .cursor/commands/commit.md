...
1. **Проверь наличие файлов в staged (git index):**
   - Если файлы есть в staged — анализируй diff между staged-файлами и предыдущим коммитом.
   - Если файлов нет в staged — анализируй измененные файлы (changed files) из `git status` и предложи тематические коммиты или один коммит.
...
2. **Анализ изменений:**
   ...
   - Если анализируешь changed файлы, группируй их по тематике (например, по типам изменений, по модулям, по функциональности).

3. **Проверь git history:**
...
4. **Если есть сомнения в трактовке изменений — уточни у пользователя, прежде чем формировать сообщение.**

5. **Сообщение коммита всегда должно быть максимально информативным и отражать реальный масштаб и суть изменений, а не только их внешний вид (например, массовая замена может быть как рефакторингом, так и внедрением нового API).**

6. **Если файлов нет в staged:**
   - Проанализируй все измененные файлы (modified, deleted, untracked).
   - Предложи пользователю варианты:
     - **Тематические коммиты**: Раздели изменения на логические группы и предложи несколько коммитов (например, один для документации, другой для рефакторинга).
     - **Один коммит**: Если изменения логически связаны, предложи один коммит со всеми изменениями.
   - После выбора варианта пользователем, добавь соответствующие файлы в staged и выполни коммит(ы).

7. **Покажи пользователю сформированное commit-сообщение и всегда запрашивай подтверждение коммита после вывода сообщения. Только после явного разрешения пользователя выполняй коммит. Согласование обязательно!!!**

8. **Публикация коммита - Используйте here-document (<<EOF ... EOF) для передачи сообщения коммита через стандартный ввод:**
